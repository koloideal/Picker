...
from get_data_telegram.get_users_from_group import get_users_from_groups
import os


async def get_url_for_users_in_groups_rout(message: types.Message, state: FSMContext):

    if message.text.startswith('t.me') or message.text.startswith('https://t.me'):

        processed = await message.answer('Processed...')

        file_name: str = await get_users_from_groups(message)
